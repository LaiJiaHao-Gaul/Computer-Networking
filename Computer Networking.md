...
步骤：
    ...
    1.DHCP服务器发现。新到达主机的首要任务是发现DHCP服务器，这通过使用 DHCP发现报文（DHCP discover message）完成，客户在UDP分组中向端口67发送该发现报文。该UDP分组封装在IP数据报中，使用广播目的地址255.255.255.255并且使用“本主机”源IP地址0.0.0.0。

    2.DHCP服务器提供。DHCP服务器收到DHCP发现报文时，用DHCP提供报文（DHCP offer message）向客户做出响应，该报文向该子网的所有节点广播，使用IP广播地址255.255.255.255（采用广播是为了让客户从几个提供者之间自由选择）。每个提供报文中含有收到的发现报文的事务ID、向客户推荐的IP地址、网络掩码一级IP地址租用期（ad-dress lease time），也就是IP地址有效的时间量。

    3.DHCP请求。客户从一个或多个服务器提供中选择一个，并且向选中的服务器提供发送DHCP请求报文（DHCP request message）进行响应，回显配置的参数。
